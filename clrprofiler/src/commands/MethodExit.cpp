...
namespace Commands
{
	MethodExit::MethodExit(__int64 timestamp, __int64 threadid, __int64 moduleid, __int64 methoddef)
		: timestamp(timestamp), code(0x0007), hasEncoded(false), ThreadID(threadid), ModuleID(moduleid), MethodDef(methoddef) 
	{
	}
	...
	std::wstring MethodExit::Name()
	{
		return L"Method Exit";
	}

	std::wstring MethodExit::Description()
	{
		return L"Update Command Description";
	}

	std::shared_ptr<std::vector<char>> MethodExit::Encode()
	{
		if (!hasEncoded)
		{
	...
	std::shared_ptr<ICommand> MethodExit::Decode(std::shared_ptr<std::vector<char>> &data)      
	{
		auto hash = std::hash<std::wstring>();
		...
		auto term = *(short*)ptr;
		ptr += sizeof(short);
		return std::make_shared<MethodExit>(local_timestamp,  local_ThreadID ,  local_ModuleID ,  local_MethodDef );
	}
}
