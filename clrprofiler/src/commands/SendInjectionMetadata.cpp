...
namespace Commands
{
	SendInjectionMetadata::SendInjectionMetadata(__int64 timestamp, std::vector<char> injectionmetadata)
		: timestamp(timestamp), code(0x0009), hasEncoded(false), InjectionMetadata(injectionmetadata) 
	{
	}
	...
	std::shared_ptr<ICommand> SendInjectionMetadata::Decode(std::shared_ptr<std::vector<char>> &data)      
	{
		...
		auto term = *(short*)ptr;
		ptr += sizeof(short);
		return std::make_shared<SendInjectionMetadata>(local_timestamp,  local_InjectionMetadata );
	}
}
